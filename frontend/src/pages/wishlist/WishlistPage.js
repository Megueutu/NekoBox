  ...
  const content = `
    <div class="space-y-6">
      <div class="flex items-center justify-between">
        <div>
          <h1 class="text-2xl font-black">Lista de Desejos</h1>
          <p class="text-zinc-500 text-sm mt-1">${wishlist.length} item${wishlist.length !== 1 ? "s" : ""} salvo${wishlist.length !== 1 ? "s" : ""}</p>
        </div>
      </div>
      ...
      ${
        ...
        <div class="bg-white border border-zinc-200 rounded-lg p-12 text-center">
          ...
          <p class="text-zinc-500 text-lg font-semibold mb-2">Sua lista de desejos está vazia</p>
          <p class="text-zinc-400 text-sm mb-6">Explore o catálogo e salve os jogos que te interessam.</p>
      ...
      `
        ...
        <div class="grid grid-cols-2 sm:grid-cols-3 xl:grid-cols-4 gap-4">
          ${wishlist
            .map(
              (game) => `
            <div class="bg-white border border-zinc-200 rounded-lg overflow-hidden group">
              <!-- Capa -->
              ...
              <div class="p-3 space-y-2">
                ...
                <p class="font-bold text-sm">${formatPrice(game.price)}</p>
                <div class="flex gap-2">
                  <button data-add-cart="${game.id}"
                          class="flex-1 py-1.5 bg-zinc-900 text-white text-xs font-bold rounded hover:bg-zinc-700 transition-colors">
                    + Carrinho
                  </button>
                  <button data-remove-wishlist="${game.id}"
                          class="px-2.5 py-1.5 border border-red-300 text-red-500 text-xs rounded hover:bg-red-50 transition-colors">
                    ✕
                  </button>
                </div>
              </div>
