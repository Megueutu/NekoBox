...
async function atualizarPagina(callback) {
...
export async function renderizar({ focusTarget = null } = {}) {
    ...
    try {
      ...
      retryButton?.addEventListener("click", () => {
        ...
        retryButton.innerHTML = `
          ...
          Tentando novamente...
        `;
        renderizar({ focusTarget: "#main-content" });
      });
      retryButton?.focus();
      return;
    }
...
export function inicializar() {
  ...
  window.addEventListener("rerender", (event) =>
    renderizar({ focusTarget: event.detail?.focusTarget })
  );
