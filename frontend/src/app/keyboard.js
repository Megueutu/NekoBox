...
export function setupKeyboardNavigation(root = document) {
  ...
  const handleKeyDown = (event) => {
    ...
    const group = event.target.closest('[role="tablist"], [role="radiogroup"], [role="group"]');
    if (!group) return;

    const role = group.getAttribute("role");
    const isToggleGroup = role === "group";
    const itemRole = role === "tablist" ? "tab" : role === "radiogroup" ? "radio" : "button";
    const items = isToggleGroup
      ? [...group.querySelectorAll("button:not([disabled])")]
      : [...group.querySelectorAll(`[role="${itemRole}"]:not([disabled])`)];
